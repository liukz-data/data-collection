...
class MroObjectFieldsSeq(
  ...
  def this() {
    this(
      0, 0, "", 0, 0, 0, "", new Array[AnyVal](MroObjectFieldsSeq.realFieldsLen)
    )
  }
...
//    if (FieldsMask.allFieldsMask.getOrElse("MmeUeS1apId", (false, null))._1) {
      ...
      dataOutput.writeByte(lenMmeUeS1apId)
...
//    if (FieldsMask.allFieldsMask.getOrElse("TimeStamp", (false, null))._1) {
      ...
      dataOutput.writeByte(lenTimeStamp)
...
//            System.exit(1)
    ...
    if ( fieldsInd != MroObjectFieldsSeq.realFieldsLen ){
...
//    }
//    if (FieldsMask.allFieldsMask.getOrElse("MmeUeS1apId", (false, null))._1) {
      val lenMmeUeS1apId = dataInput.readByte()
...
//    }
//    if (FieldsMask.allFieldsMask.getOrElse("TimeStamp", (false, null))._1) {
      val lenTimeStamp = dataInput.readByte()
...
//    }
    ...
    // mrofields: mro_sc, mro_nc, mro_2nd, mro_3nd
    val fieldsArr: Array[AnyVal] = new Array[AnyVal](MroObjectFieldsSeq.realFieldsLen)
...
//                " ERROR MroObjectFieldsSeq: 反序列化时，日志字段信息的JSON文件描述有错误，问题字段是" + FieldsMask.allFields(i))
          ...
          case iobe: java.lang.IndexOutOfBoundsException =>
            val e = new Exception("反序列化过程中，实际数据的字段数与日志字段信息JSON文件中的字段数目不一致。出错数据的MroKey是：\n"
              ...
              MmeUeS1apId.toString(), MmeGroupId.toString, MmeCode.toString, TimeStamp)/*+"\n"+"实际数据的值：+\n"
              +String.join(",",ENBID.toString, ECI.toString, CGI,
              MmeUeS1apId.toString(), MmeGroupId.toString, MmeCode.toString, TimeStamp,fieldsArr.toBuffer.toString())+"\n"
            +"实际数据的长度："+(8+fieldsArr.length).toString+"\n"
            +"json的信息：\n"+System.getProperty("SYS_MROOBJ_JSON")*/)
            e.setStackTrace(iobe.getStackTrace)
            throw e
//            System.err.print(sm.format(new Date()) +
