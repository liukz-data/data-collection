...
object MergeSeqFilesRepartitionOneHour{
  ...
  private[merge] def executeMerge(args: Array[String], hadoopConfMap: Map[String, String] = null): Unit = {
    ...
    var date:String = "20181211"
    var hour:String = "00"
    if (args.length != 5) {
      System.err.println("使用方法: com.com.merge.MergeSeqFiles <配置文件路径> [<Spark日志级别，默认为error,city,date,hour>]")
      System.exit(1)
    } else {
      confFile = args(0)
      if (args.length == 2)
    ...
    val regionNum = getCompletedSeqFiles(city,date,hour,seqfile_root)
...
//    System.setProperty("SYS_MROOBJ_JSON", jsonContent)
    ...
    val sparkConf = new SparkConf().setAppName("MergeSeqFiles")
      ...
      .set("spark.shuffle.sort.bypassMergeThreshold","30000")
    ...
    val hadoopConfNewApi = new Configuration()
    val minSize = 1024*1024*128
    val splitMinSize = prop.getProperty("mapreduce.input.fileinputformat.split.minsize")
    val splitMaxSize = prop.getProperty("mapreduce.input.fileinputformat.split.maxsize")
    hadoopConfNewApi.set("mapreduce.input.fileinputformat.split.minsize",splitMinSize)
    hadoopConfNewApi.set("mapreduce.input.fileinputformat.split.maxsize",splitMaxSize)
//  val job =Job.getInstance()

   // sc.newAPIHadoopFile("",classOf[CombineSequenceFileInputFormat[NullWritable,MroObjectFieldsSeq]],classOf[NullWritable],classOf[MroObjectFieldsSeq],hadoopConf1)
    ...
    val compFile = String.join("_",city,date,hour)
      try {
...
//        val mroObjRDD = getMroObjRDDDyn(sc, seqfile_root, city_date_hour(0), city_date_hour(1), city_date_hour(2))
        val mroObjRDD = {
          ...
          val logRdd: RDD[MroObjectFieldsSeq] = sc.newAPIHadoopFile(String.join("/", seqfile_root,
            city, date, hour),classOf[CombineSequenceFileInputFormat[NullWritable,MroObjectFieldsSeq]],classOf[NullWritable],classOf[MroObjectFieldsSeq],hadoopConfNewApi).map(row => row._2)
        ...
        results.repartition(regionNum).write
          .format("orc")
        ...
        val seqFileLoc = seqfile_root + "/" + city + "/" + date + "/" + hour
  ...
  /**
    ...
    * @return
    */
  private def getCompletedSeqFiles(city: String,date:String,hour:String,seqfile_root:String): Int = {
    try {
      ...
      //  var arrCompSeqFile = new Array[(String,Int)](len)
       // for (i <- 0 until len) {
         ...
         // val city_date_hour = fileName.split("_")
          val seqfile_root_city_date_hour = String.join("/", seqfile_root,city, date, hour)
