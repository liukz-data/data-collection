...
/**
  * MergeSeqFilesCoalesceSerialLine会将/mro/comp_seqfiles下所有city_date_hour文件读取过来
  * 解析其内部is_to_hive=true/false,if_to_analyze_mro=true/false等参数配置
  * 若city_date_hour文件内参数is_to_hive=true则进行入库，否则不入库
  * 若city_date_hour文件内参数if_to_analyze_mro=true并把city_date_hour移动到comp_merge文件夹下，否则删除city_date_hour文件
  *
  */
object MergeSeqFilesCoalesceSerialLine{
  ...
  private[merge] def executeMerge(args: Array[String], hadoopConfMap: Map[String, String] = null): Unit = {
    ...
    try {
      ...
      PgMergeLogicTb.init(pgdb_ip, pgdb_port.toInt, pgdb_user,
        DecryptTool.getDecryptString(pgdb_passwd), pgdb_db)
    } catch {
      case ex: Exception =>
    ...
    val stage_delay_submit_time = prop.getProperty("stage_delay_submit_time").toInt*1000
    // 创建 FileSystem 实例
...
//    System.setProperty("SYS_MROOBJ_JSON", jsonContent)
    ...
    val comp_merge = prop.getProperty("comp_merge")
    output_seqfiles.filter(compFileAndRegionNum=>{
      val compFile = compFileAndRegionNum._1
      val if_to_analyze_mro = compFileAndRegionNum._3
      val is_to_hive = compFileAndRegionNum._5

      val compFilePath = new Path(String.join("/",comp_seqfiles,compFile))
      if(is_to_hive == "false"){
        if(if_to_analyze_mro == "true") {
          println(sm.format(new Date()) + s" INFO MergeSeqFiles: move $compFilePath $comp_merge")
          fileSystem.rename(compFilePath,new Path(comp_merge))
        }else{
          println(sm.format(new Date()) + s" INFO MergeSeqFiles: delete $compFilePath")
          fileSystem.deleteOnExit(compFilePath)
        }
      }
      is_to_hive.toBoolean
    }).par.foreach(compFileAndRegionNum => {
      val compFile = compFileAndRegionNum._1
      val regionNum = compFileAndRegionNum._2
      val if_to_analyze_mro = compFileAndRegionNum._3
      val taskid = compFileAndRegionNum._4
      ...
      try {
        ...
        val hourInt = city_date_hour(2).toInt
        val delayTime = hourInt*stage_delay_submit_time
        println("延迟提交Job时间："+delayTime)
        ...
        PgMergeLogicTb.insert(taskid,city_date_hour(0),city_date_hour(1),city_date_hour(2),"merge","start")
//        val mroObjRDD = getMroObjRDDDyn(sc, seqfile_root, city_date_hour(0), city_date_hour(1), city_date_hour(2))
        val mroObjRDD = {
          ...
          val partitions = logRdd.getNumPartitions
        ...
        // println("stage_parallelism:"+stage_parallelism)
       ...
       val compFilePath = new Path(String.join("/",comp_seqfiles,compFile))
        ...
        PgMergeLogicTb.insert(taskid,city_date_hour(0),city_date_hour(1),city_date_hour(2),"merge","stop")
        //
       ...
       // 7. 删除 已经完成合并的 SequenceFile小文件
        ...
        case ex: Exception =>
          ...
          msgList3.append((sm.format(new Date()), compFile,
            "ERROR MergeSeqFiles: 合并数据出现错误，数据合并程序退出！\n"+exceptionMsg))
          PgMergeLogicTb.insert(taskid,city_date_hour(0),city_date_hour(1),city_date_hour(2),"merge","error")
      } finally {
  ...
  /**
    ...
    * @return
    */
  private def getCompletedSeqFiles(compSeqFilePath: String,seqfile_root:String): Array[(String,Int,String,Int,String)] = {
    try {
      ...
      if (len == 0) {
        ...
        var arrCompSeqFile = new Array[(String,Int,String,Int,String)](len)
        for (i <- 0 until len) {
          val filePath = filesStatus(i).getPath
          val fileName = filePath.getName
          val in = fileSystem.open(filePath,1024)
          val pro = new Properties()
          pro.load(in)
          val if_to_analyze_mro = pro.getProperty("if_to_analyze_mro","false")
          val taskid = pro.getProperty("taskid","2").toInt
          val is_to_hive = pro.getProperty("is_to_hive","false")

         if(if_to_analyze_mro.toBoolean){
           ...
           arrCompSeqFile(i) =(fileName,regionNum.toInt,if_to_analyze_mro,taskid,is_to_hive)
           sb.append(fileName + " \n")
         }else{
           arrCompSeqFile(i) =(fileName,-1.toInt,if_to_analyze_mro,taskid,is_to_hive)
         }
