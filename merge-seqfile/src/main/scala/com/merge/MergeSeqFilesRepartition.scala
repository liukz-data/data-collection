...
import org.apache.hadoop.mapreduce.lib.input.{CombineSequenceFileInputFormat, KeyValueTextInputFormat, SequenceFileInputFormat}
...
object MergeSeqFilesRepartition{
...
//        val mroObjRDD = getMroObjRDDDyn(sc, seqfile_root, city_date_hour(0), city_date_hour(1), city_date_hour(2))
        val mroObjRDD = {
          ...
          val logRdd: RDD[MroObjectFieldsSeq] = sc.newAPIHadoopFile(String.join("/", seqfile_root,
            city_date_hour(0), city_date_hour(1), city_date_hour(2)),classOf[CombineSequenceFileInputFormat[NullWritable,MroObjectFieldsSeq]],classOf[NullWritable],classOf[MroObjectFieldsSeq],hadoopConfNewApi).map(row => row._2)
