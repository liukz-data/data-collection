...
#      无
# 运行命令：
#      sh merge-data-start-one-hour-coalesce.sh city date hour
###################################################################################
...
if [[ $# -lt 3 ]]
then
  echo "使用方法：merge-data-start-one-hour.sh [配置文件mromerge-conf.properties的路径 city date hour]"
elif [[ $# -eq 3 ]]
then
  city=$1
  date=$2
...
spark-submit \
             ...
             --conf "spark.executor.extraJavaOptions=-verbose:gc -XX:+PrintGCDetails -XX:+PrintGCDateStamps -XX:+UseG1GC -XX:ParallelGCThreads=5 -XX:ConcGCThreads=2 -XX:InitiatingHeapOccupancyPercent=40" \
             --jars ../libs/generate-schemacls-1.0.jar,../libs/postgresql-42.2.5.jar \
             --class com.merge.MergeSeqFilesCoalesceOneHour \
             ../libs/merge-seqfile.jar ${mromerge_conf_hdfs} ${log_level} ${city} ${date} ${hour} 1>> ${log_file} 2>> ${log_file}
